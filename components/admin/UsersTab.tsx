...
export function UsersTab() {
  ...
  const [selectedUserForPerms, setSelectedUserForPerms] =
  ...
  const confirmResetPassword = async () => {
    ...
    if (res?.success) {
      toast.success(
        `Password reset for ${userToReset.name}. New Password: ${res.password}`,
      );
  ...
  const handleLoginAs = async (user: UserRecord) => {
    if (
      !confirm(
        `Are you sure you want to login as ${user.name}? This will end your current admin session.`,
      )
    )
      return;
  ...
  const handleSavePermissions = async () => {
    ...
    const res = await updateUser(selectedUserForPerms.id, {
      ...
      role: selectedUserForPerms.role as
        | "STAFF"
        | "HUB_ADMIN"
        | "TENANT_ADMIN"
        | "SUPER_ADMIN"
              ...
              >
                ...
                <td className="p-4 text-xs text-gray-500 max-w-[250px]">
                  <div className="flex flex-col gap-2 items-start">
                        ...
                        user.permissions.map((perm: string) => {
                          ...
                          return (
                            <Badge
                              key={perm}
                              variant="outline"
                              className={`${permObj?.color || "bg-slate-50 text-slate-600 border-slate-200"} font-normal`}
                            >
                              {permObj?.label || perm}
                            </Badge>
                          );
                    ...
                    {["TENANT_ADMIN", "HUB_ADMIN", "STAFF"].includes(
                      user.role,
                    ) && (
                      <Button
                        size="sm"
                ...
                <td className="p-4">
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent
                      align="end"
                      className="w-[200px] bg-white border border-gray-100 shadow-xl rounded-lg p-1"
                    >
                      <DropdownMenuItem
                      ...
                      <DropdownMenuItem
                        ...
                        className="cursor-pointer hover:bg-gray-50 focus:bg-gray-50 rounded px-2 py-2 text-sm text-gray-700 flex items-center gap-2"
                      >
                        <LogIn className="w-4 h-4 text-gray-500" /> Login As
                        Company
                      </DropdownMenuItem>
                      <DropdownMenuItem
                      ...
                      <div className="flex border border-slate-200 rounded-md shadow-sm bg-white overflow-hidden focus-within:ring-2 focus-within:ring-slate-900 focus-within:ring-offset-2">
                        <input
                          ...
                          onChange={(e) => {
                            ...
                            const formattedId = baseName.trim()
                              ? `${baseName.trim().toLowerCase().replace(/\s+/g, "-")}-hub`
                              : "";
            ...
            <div className="px-6 py-4 border-b border-gray-100 flex justify-between items-center bg-gray-50">
              <div>
                ...
                <p className="text-xs text-gray-500">
                  For {selectedUserForPerms.name} (
                  {{
                    SUPER_ADMIN: "Super Admin",
                    TENANT_ADMIN: "Shop Admin",
                    HUB_ADMIN: "Hub Manager",
                    STAFF: "Staff",
                  }[selectedUserForPerms.role as string] ||
                ...
                <div className="grid grid-cols-1 gap-3">
                  {AVAILABLE_PERMISSIONS.map((perm) => (
                    ...
                    >
                      ...
                      <div>
                        <span
                          className={`font-medium text-sm ${tempPermissions.includes(perm.id) ? "text-current" : "text-slate-700"}`}
                        >
                          {perm.label}
                        </span>
                      </div>
              ...
              <div className="space-y-4">
                <div className="space-y-1.5">
                  ...
                  <p className="text-[10px] text-gray-400">
                    If blank, a secure random password will be generated.
                  </p>
                </div>
                <div className="pt-4 flex justify-end gap-3">
                  <Button

...
import { fetchEventsByDuration } from "@/app/[domain]/admin/event-exports/actions";
...
export function EventExportButton() {
  ...
  const handleExport = async (days: number) => {
    ...
    const toastId = toast.loading(
      `Generating Event Matrix report for last ${days} days...`,
    );

    try {
      const events = await fetchEventsByDuration(days);

      if (!events || events.length === 0) {
        toast.error(`No events found for the last ${days} days.`, {
          id: toastId,
        });
        return;
      }
      ...
      toast.success(`Success! ${events.length} events exported.`, {
        id: toastId,
      });
    } catch (error) {
  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <Button
          variant="outline"
          className="text-xs font-bold uppercase tracking-wider h-10 gap-2 border-slate-200 hover:bg-slate-50 shadow-sm"
      ...
      >
        <DropdownMenuLabel className="text-slate-500 text-[10px] font-black uppercase tracking-widest px-3">
          Select Duration
        </DropdownMenuLabel>
        <DropdownMenuSeparator className="bg-slate-100" />
        <DropdownMenuItem
          onClick={() => handleExport(7)}
          className="cursor-pointer font-bold text-slate-700 text-xs py-2.5"
        >
          Last 7 Days
        </DropdownMenuItem>
        <DropdownMenuItem

...
const getSectionStyle = (index: number, isDragging: boolean) => {
  ...
  if (isDragging)
    return "bg-white shadow-xl scale-[1.01] z-50 ring-1 ring-slate-200 border-l-orange-500 opacity-100";

  // Gradient logic: Top (Green) -> Middle (Yellow) -> Bottom (Subtle Red/Stone)
  if (index === 0)
    return "bg-emerald-50/40 border-l-emerald-500/50 hover:border-l-emerald-500";
  if (index === 1)
    return "bg-amber-50/40 border-l-amber-500/50 hover:border-l-amber-500";
...
function SortableSection({
  ...
  readonly onUpdate: (id: string, updates: Partial<Section>) => void;
}) {
  const {
    attributes,
    listeners,
    ...
    >
      ...
      >
        <div
          {...listeners}
          {...attributes}
          className="cursor-grab active:cursor-grabbing text-slate-400 hover:text-slate-600 p-2 hover:bg-black/5 rounded-md transition-colors"
        >
        ...
        <div className="flex-1">
          ...
          <div className="text-xs text-slate-500 flex flex-wrap items-center gap-2 px-2 mt-1">
            <div className="flex items-center bg-white border border-slate-200 rounded-md shadow-sm h-7 max-w-[250px] overflow-hidden focus-within:ring-1 focus-within:ring-orange-500 focus-within:border-orange-500">
              <span className="pl-2 pr-1 text-slate-400 select-none">
                /menu?section=
              </span>
              <input
                value={section.slug}
            ...
            <span
              className={
                section._count?.products === 0 ? "text-red-500 font-medium" : ""
              }
            >
              {section._count?.products || 0} Products
              {(!section._count?.products || section._count?.products === 0) &&
                " (Hidden on Home)"}
            </span>
          </div>
        </div>
...
export function SectionsManager() {
  ...
  const fetchSections = async () => {
    setLoading(true);
    let data = await getSections(domain);

    if (data.length === 0) {
      try {
        await seedDefaultSections(domain);
        data = await getSections(domain);
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500">
            Drag to prioritize sections on the home page.
          </p>
        </div>
        <div className="flex gap-2">
          <Button variant="outline" size="sm" onClick={handleSeed}>
            <RefreshCw className="w-4 h-4 mr-2" /> Re-Seed Defaults
          </Button>
          <Button

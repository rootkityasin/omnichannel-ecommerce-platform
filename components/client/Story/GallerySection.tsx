...
import type { GalleryItem, GallerySectionContent } from "@/types/common";

interface GalleryProps {
  data: GallerySectionContent | GalleryItem[] | null;
}
...
export function GallerySection({ data }: GalleryProps) {
  ...
  const galleryImages = isLegacyArray
    ? data.length > 0
      ? data
      : defaultImages
        ...
        {galleryImages.map((img, index) => (
          ...
          >
            <img
              src={
                img.src ||
                "https://placehold.co/400x500/1e293b/FFF?text=No+Image"
              }
              alt={img.alt}

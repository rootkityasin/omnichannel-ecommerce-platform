...
export const getCategoryStyle = (
  ...
  const getRotation = (magnitude: 3 | 6 | 12) => {
    if (magnitude === 12)
      return isRight ? "group-hover:rotate-12" : "group-hover:-rotate-12";
    if (magnitude === 6)
      return isRight ? "group-hover:rotate-6" : "group-hover:-rotate-6";
  ...
  switch (type) {
    ...
    case "FLIP":
      rotateClass =
        "group-hover:[transform:rotateY(180deg)] transition-transform duration-500";
      break;
    case "AUTO":
    default:
  ...
  return (
    <section className="relative py-4 md:py-16 overflow-hidden">
      ...
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full h-full max-w-7xl">
        ...
        <div
          className="absolute bottom-0 left-0 w-96 h-96 bg-orange-400/5 rounded-full blur-[120px] -z-10 animate-pulse"
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-6 md:mb-12 gap-4">
          ...
          >
            ...
            <h2 className="text-3xl md:text-5xl font-black text-slate-900 tracking-tight leading-none">
              Our{" "}
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-crab-red to-orange-600">
                Categories
              </span>
            </h2>
          </motion.div>
            ...
            return (
              ...
              >
                <div className="relative">
                  <div
                    className={`w-14 h-14 sm:w-16 sm:h-16 rounded-xl sm:rounded-2xl bg-white shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-100 flex items-center justify-center ${style.color} group-active:scale-95 transition-all duration-300`}
                  >
                    <Icon className="w-6 h-6 sm:w-7 sm:h-7 transition-transform group-active:scale-110" />
                  </div>
                  <div
                    className={`absolute inset-0 opacity-0 group-active:opacity-10 rounded-xl sm:rounded-2xl transition-opacity bg-current ${style.color}`}
                  />
                </div>
            ...
            return (
              ...
              >
                ...
                >
                  ...
                  <div
                    className={`absolute inset-0 bg-gradient-to-br ${style.gradient} to-transparent opacity-0 group-hover:opacity-100 rounded-3xl blur-2xl transition-opacity duration-500`}
                  />

                  <div
                    className={`relative h-full flex flex-col items-center justify-center p-8 bg-white/60 backdrop-blur-xl border border-white/80 rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] ${style.shadow} group-hover:-translate-y-3 group-hover:bg-white/90 transition-all duration-500 overflow-hidden`}
                  >
                    {/* Content */}
                    <div className="relative z-10 flex flex-col items-center">
                      <div
                        className={`mb-6 p-5 rounded-2xl bg-slate-50 ${style.color} ${style.bg} group-hover:text-white shadow-inner transition-all duration-500 transform ${style.rotate}`}
                      >
                        <Icon className="w-8 h-8" />
                      </div>
                      <h3
                        className={`text-sm font-black text-slate-900 ${style.hoverText} text-center uppercase tracking-widest transition-colors duration-300`}
                      >
                        {cat.name}
                      </h3>
                    </div>
                    ...
                    {/* Arrow Indicator */}
                    <div
                      className={`absolute bottom-4 right-4 opacity-0 group-hover:opacity-100 transform translate-x-4 group-hover:translate-x-0 transition-all duration-500 ${style.color}`}
                    >
                    ...
                    {/* Bottom Accent */}
                    <div
                      className={`absolute bottom-0 left-1/2 -translate-x-1/2 w-0 h-1 bg-current ${style.color} group-hover:w-full transition-all duration-500`}
                    />
                  </div>
                </Link>
              </motion.div>

...
export function AuthForm() {
  ...
  const [inputType, setInputType] = useState<"PHONE" | "EMAIL" | "UNKNOWN">(
    "UNKNOWN",
  );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (isLogin) {
      ...
      if (!formData.contact) newErrors.contact = "Contact is required";
        ...
        inputType === "PHONE" &&
        !/^(\+88)?01[3-9]\d{8}$/.test(formData.contact)
      )
        newErrors.contact = "Invalid BD Mobile Number";
      else if (inputType === "EMAIL" && !formData.contact.includes("@"))
      ...
      if (isLogin) {
        ...
        if (res.success) {
          ...
          if (loginRes?.error) {
            ...
            if (
              role &&
              ["SUPER_ADMIN", "HUB_ADMIN", "TENANT_ADMIN", "STAFF"].includes(
                role,
              )
            ) {
              router.push("/admin");
    ...
    >
      ...
      <div className="grid grid-cols-1 gap-4 mb-6">
        <Button
          ...
          disabled={isLoading}
          className="h-12 rounded-2xl border-0 shadow-sm ring-1 ring-gray-100 hover:bg-gray-50 hover:ring-crab-red/20 transition-all text-gray-900 font-bold"
        >
      ...
      <div className="relative mb-6">
        ...
        <div className="relative flex justify-center text-[10px] uppercase cursor-default">
          <span className="bg-gray-50 px-2 text-gray-400 font-bold tracking-widest">
            Or continue with
          </span>
        </div>
      </div>
      ...
      <div className="mt-6 text-center space-y-3">
        ...
        <div className="text-[10px] text-gray-400 font-medium font-body px-8 leading-tight">
          ...
          <span className="text-gray-600 hover:text-crab-red underline cursor-pointer font-bold transition-colors">
            Terms of Service
          </span>{" "}
          and{" "}
          <span className="text-gray-600 hover:text-crab-red underline cursor-pointer font-bold transition-colors">

    ...
    <>
      <header
        className={cn(
          ...
          isSidebarOpen
            ? "z-[100] bg-transparent backdrop-blur-none delay-100"
            : "z-50",
      ...
      >
        <div className="max-w-7xl mx-auto px-8 flex items-center justify-between h-full relative">
          ...
          <motion.nav
            ...
            className={cn(
              "hidden lg:flex items-center gap-8 bg-white/50 backdrop-blur-md px-8 py-2.5 rounded-full border border-slate-200/20 shadow-sm transition-opacity duration-300",
              isSidebarOpen && "opacity-0 pointer-events-none",
              "absolute left-1/2 top-1/2",
            )}
            style={{ maxWidth: "clamp(22rem, 46vw, 38rem)" }}
          >
            {navItems.map((item) => {

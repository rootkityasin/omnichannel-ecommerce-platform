  ...
  return (
    ...
    >
      ...
      <div className="container mx-auto px-4 relative z-10">
        <div className="flex items-center justify-between mb-8">
          ...
          >
            <h2 className="text-2xl md:text-3xl font-black text-slate-900 tracking-tight flex items-center gap-3">
              {title}
              {!enableScrollAnimation && (
                <span className="w-8 h-1 bg-crab-red rounded-full" />
              )}
            </h2>
          </motion.div>
          ...
          <Link
            href={viewAllLink}
            className="group flex items-center gap-2 px-4 py-2 rounded-full bg-slate-100 text-slate-900 text-xs font-black uppercase tracking-widest hover:bg-crab-red hover:text-white transition-all duration-300"
          >
            View All
        ...
        <div
          className={`overflow-x-auto pb-4 hide-scrollbar md:hidden ${enableScrollAnimation ? "" : "snap-x"}`}
        >
          <motion.div

        ...
        <div className="h-[calc(100vh-4rem)] flex flex-col">
            ...
            <div className="flex justify-between items-center mb-6 px-1">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="text-2xl font-bold tracking-tight text-slate-800">Invoice</h1>
                        <p className="text-sm text-slate-500">Customize your receipt layout and style.</p>
                    </div>
                </div>
                <Button onClick={handleSave} disabled={isSaving} className="bg-slate-900 hover:bg-orange-600">
            ...
            <div className="flex flex-col lg:flex-row gap-6 h-full overflow-hidden">
                ...
                <div className="w-full lg:w-80 flex-shrink-0 space-y-6 overflow-y-auto pb-10 pr-2">
                    <Card className="p-4 space-y-4">
                        <h3 className="font-semibold text-sm uppercase tracking-wider text-slate-500">General</h3>

                        <div className="space-y-2">
                            <Label>Theme Style</Label>
                            <Select
                                value={config.invoiceTheme}
                                onValueChange={(val) => setConfig({ ...config, invoiceTheme: val })}
                            >
                                <SelectTrigger>
                    ...
                    <Card className="p-4 space-y-4">
                        <h3 className="font-semibold text-sm uppercase tracking-wider text-slate-500">Visibility</h3>
                    ...
                    <Card className="p-4 space-y-4">
                        <h3 className="font-semibold text-sm uppercase tracking-wider text-slate-500">Branding</h3>

                        <div className="space-y-4">
                            <Label>Watermark Opacity ({Math.round(config.invoiceDetails.watermarkOpacity * 100)}%)</Label>
                            <Slider
                ...
                <div className="flex-1 bg-slate-100 rounded-xl overflow-hidden shadow-inner flex flex-col">
                    <div className="bg-white border-b p-2 flex justify-end">
                        <span className="text-xs text-slate-400 uppercase font-bold tracking-widest px-3 py-1">Live Preview</span>
                    </div>
                        ...
                        >
                            ...
                            <div className="relative z-10 p-8 h-full flex flex-col">
                                    ...
                                    <>
                                        ...
                                        <div className="flex justify-between items-end border-b-4 border-slate-800 pb-2 mb-4">
                                            ...
                                            <div className="text-right">
                                                ...
                                                <p className="text-xs text-slate-500">Doc No: {dummyOrder.orderId}</p>
                                            </div>
                                        </div>
                                        ...
                                        <div className="flex gap-4 mb-6">
                                            {config.invoiceDetails.showBuyer && (
                                                <div className="flex-1 border border-slate-300 p-2">
                                                    ...
                                                    <p className="text-sm">{dummyOrder.customerAddress}</p>
                                                    <p className="text-sm font-mono mt-1">{dummyOrder.customerPhone}</p>
                                                </div>
                                            )}
                                        ...
                                        <div className="border border-slate-400 flex text-sm mb-6 bg-slate-50">
                                            <div className="flex-1 p-1 border-r border-slate-400 px-2">Order Number: <strong>{dummyOrder.orderId}</strong></div>
                                            <div className="flex-1 p-1 border-r border-slate-400 px-2 text-center">Payment: <strong>Cash on Delivery</strong></div>
                                            <div className="flex-1 p-1 px-2 text-right">Date: <strong>{dummyOrder.date}</strong></div>
                                        </div>
                                        ...
                                        <table className="w-full text-sm border-collapse border border-slate-300 mb-6">
                                            ...
                                            <tbody>
                                                ...
                                                <tr>
                                                    ...
                                                    <td className="text-right p-2 bg-slate-50">{dummyOrder.subtotal}</td>
                                                </tr>
                                                <tr>
                                                    <td colSpan={4} className="text-right p-2 font-bold bg-slate-50 border-r border-slate-300">Shipping</td>
                                                    <td className="text-right p-2 bg-slate-50">{dummyOrder.delivery}</td>
                                                </tr>
                                                <tr>
                                                    <td colSpan={4} className="text-right p-2 font-bold bg-slate-100 border-r border-slate-300 text-base">Total Payable Amount</td>
                                                    <td className="text-right p-2 bg-slate-100 font-bold text-base border-b-2 border-slate-800">{dummyOrder.total}</td>
                                                </tr>
                                            </tbody>
                                        </table>
                                            ...
                                            <div>
                                                {config.invoiceDetails.showQr && (
                                                    <div className="flex flex-col items-center gap-1">
                                                        ...
                                                        <span className="text-[10px] font-mono">{dummyOrder.orderId}</span>
                                                    </div>
                                                )}
                                            </div>
                                            <div className="text-right text-xs text-slate-500">
                                                <p>**This is a computer generated copy. No signature is required**</p>
                                            </div>
                                        </div>
                                    </>
                                )}
                                ...
                                {config.invoiceTheme === 'modern' && (
                                    <>
                                        <div className="absolute top-0 left-0 w-full h-1.5" style={{ backgroundColor: settings?.primaryColor || '#F40000' }}></div>
                                        ...
                                        <div className="flex justify-between items-start mb-10 mt-2">
                                            <div>
                                                {config.invoiceDetails.showLogo && settings?.logoUrl ? (
                                                    ...
                                                    <h2 className="text-xl font-bold" style={{ color: settings?.primaryColor }}>{settings?.shopName}</h2>
                                                )}
                                                <h1 className="text-4xl font-bold text-slate-900 tracking-tight">Invoice</h1>
                                                <p className="text-slate-500 font-medium">#{dummyOrder.orderId}</p>
                                            </div>
                                            {config.invoiceDetails.showSeller && (
                                                <div className="text-right text-sm text-slate-500">
                                                    <p className="font-bold text-slate-900">{settings?.shopName}</p>
                                        ...
                                        <div className="grid grid-cols-2 gap-8 mb-8 border-t border-b border-slate-100 py-6">
                                            {config.invoiceDetails.showBuyer && (
                                                <div>
                                                    ...
                                                    <p className="font-bold text-slate-800">{dummyOrder.customerName}</p>
                                                    <p className="text-sm text-slate-500">{dummyOrder.customerAddress}</p>
                                                    <p className="text-sm text-slate-500">{dummyOrder.customerPhone}</p>
                                                </div>
                                            )}
                                            <div className="text-right">
                                                <p className="text-xs font-bold uppercase tracking-wider text-slate-400 mb-2">Details</p>
                                                <p className="text-sm"><span className="text-slate-400">Date:</span> {dummyOrder.date}</p>
                                                <p className="text-sm"><span className="text-slate-400">Payment:</span> Cash on Delivery</p>
                                            </div>
                                        </div>
                                        ...
                                        <div className="flex justify-end border-t border-slate-100 pt-6">
                                            <div className="w-64 space-y-2">
                                                <div className="flex justify-between text-sm">
                                                    ...
                                                    <span className="font-bold">{dummyOrder.subtotal}</span>
                                                </div>
                                                <div className="flex justify-between text-sm">
                                                    <span className="text-slate-500">Delivery</span>
                                                    <span className="font-bold">{dummyOrder.delivery}</span>
                                                </div>
                                        ...
                                        <div className="mt-auto pt-10 flex justify-between items-end">
                                            {config.invoiceDetails.showSignature && (
                                                <div className="text-center pt-2 border-t border-slate-200 w-32">
                                                    <p className="text-[10px] uppercase font-bold text-slate-400">Authorized By</p>
                                                </div>
                                            )}
                                            {config.invoiceDetails.showQr && (
                                                <div className="flex flex-col items-center gap-1">
                                                    <QRCodeSVG value={dummyOrder.orderId} size={80} />
                                                    <span className="text-[9px] uppercase font-bold text-slate-400 tracking-widest">Order Verification</span>
                                                </div>
                                            )}
                                        </div>
                                    </>
                                ...
                                {config.invoiceTheme === 'minimal' && (
                                    <div className="text-center">
                                        <h1 className="text-3xl font-light mb-2">{settings?.shopName}</h1>
                                        <p className="text-sm text-slate-500 mb-8 lowercase tracking-wide">receipt #{dummyOrder.orderId} • {dummyOrder.date}</p>
                                        ...
                                        <div className="border border-slate-200 rounded-lg p-6 mb-8 text-left">
                                            {dummyOrder.items.map((item, i) => (
                                                <div key={i} className="flex justify-between py-2 border-b border-dashed border-slate-100 last:border-0">
                                                    <span className="font-medium">{item.name} <span className="text-slate-400 text-xs">x{item.quantity}</span></span>
                                        ...
                                        <div className="flex justify-center flex-col items-center gap-4">
                                            {config.invoiceDetails.showQr && <QRCodeSVG value={dummyOrder.orderId} size={64} />}

...
function CategoryCardPreview({
  ...
  animationType,
  iconName,
}: Readonly<{ name: string; animationType: string; iconName: string }>) {
  ...
  return (
    <div className="w-full flex justify-center py-4 bg-slate-50/50 rounded-lg border border-dashed border-slate-200">
      <div className="group relative w-32 h-32">
        <div
          className={`relative h-full flex flex-col items-center justify-center p-4 bg-white border border-slate-100 rounded-2xl shadow-sm ${style.shadow} transition-all duration-500 overflow-hidden`}
        >
          ...
          <div className="relative z-10 flex flex-col items-center gap-2">
            <div
              className={`p-2 rounded-xl bg-slate-50 ${style.color} ${style.bg} group-hover:text-white shadow-inner transition-all duration-500 transform ${style.rotate}`}
            >
              <Icon className="w-6 h-6" />
            </div>
            <span
              className={`text-[10px] font-black text-slate-900 ${style.hoverText} text-center uppercase tracking-wider transition-colors duration-300`}
            >
              {name || "Category"}
            </span>
          </div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500">
            Manage Categories and Home Page Sections.
          </p>
        </div>
      </div>

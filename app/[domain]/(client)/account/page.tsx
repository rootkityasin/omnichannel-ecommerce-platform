  ...
  useEffect(() => {
    ...
    const checkAuth = async () => {
      ...
      if (status === "authenticated" && session?.user) {
        ...
        if (
          role &&
          ["SUPER_ADMIN", "HUB_ADMIN", "TENANT_ADMIN", "STAFF"].includes(role)
        ) {
          router.push("/admin");
          return;
        }
    ...
    <div className="bg-gray-50 min-h-screen pb-20 md:pb-0 pt-0 md:pt-28">
      <div className="max-w-4xl mx-auto px-4 md:px-0">
        <motion.div
          ...
          className="bg-crab-red text-white p-6 pt-12 md:pt-16 pb-8 rounded-b-[2rem] shadow-xl mb-6 relative overflow-hidden"
        >
          ...
          <div className="relative z-10 flex flex-col items-center">
            ...
            <h1
              className={`text-xl font-bold tracking-tight mb-0.5 ${headingFontClass}`}
            >
              {user.name}
            </h1>
            <p className={`text-white/60 font-medium text-xs ${bodyFontClass}`}>
              {user.phone}
            </p>
        ...
        >
          <div className="bg-white p-4 rounded-2xl shadow-lg shadow-gray-200/50 border border-gray-100 grid grid-cols-2 gap-4">
            <div className="text-center p-2 rounded-xl hover:bg-gray-50 transition-colors hover:scale-105 duration-200">
              <span
                className={`block text-xs font-bold text-gray-400 uppercase tracking-widest mb-1 ${bodyFontClass}`}
              >
                {t.profile.points}
              </span>
              <span className="block text-2xl font-black text-crab-red">
                {formData.points}
              </span>
            </div>
          ...
          <div className="space-y-4">
            <h3
              className={`text-sm font-bold text-gray-400 uppercase tracking-wider px-2 ${bodyFontClass}`}
            >
              Account
            </h3>
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden divide-y divide-gray-50">
              <Link
                href="/account/orders"
                className="flex items-center justify-between p-4 hover:bg-gray-50 active:bg-gray-100 transition-all hover:scale-[1.01] active:scale-[0.99] group"
              >
                <div className="flex items-center gap-4">
                  ...
                  <span
                    className={`font-semibold text-gray-700 group-hover:text-gray-900 transition-colors ${bodyFontClass}`}
                  >
                    {t.profile.myOrders}
                  </span>
                </div>
          ...
          <div className="space-y-4">
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden divide-y divide-gray-50">
              ...
              <button
                onClick={onLogout}
                className="w-full flex items-center justify-between p-4 hover:bg-red-50/50 active:bg-red-50 transition-all hover:scale-[1.01] active:scale-[0.99] group text-left"
              >
                <div className="flex items-center gap-4">
                  ...
                  <span
                    className={`font-semibold text-gray-700 group-hover:text-red-700 transition-colors ${bodyFontClass}`}
                  >
                    {t.profile.logout}
                  </span>
                </div>

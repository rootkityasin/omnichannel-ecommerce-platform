...
import { getTenantUsers, updateTenantUserStatus } from '@/app/actions/super-admin';
...
export function CompanyUsersModal({ tenantId, tenantName, open, onOpenChange }: CompanyUsersModalProps) {
    ...
    const handleToggleStatus = (userId: string, currentStatus: boolean) => {
        ...
        startTransition(async () => {
            const res = await updateTenantUserStatus(userId, !currentStatus);
            if (!res.success) {
                toast.error("Failed to update status");
                // Revert
                setUsers(prev => prev.map(u => u.id === userId ? { ...u, isBlocked: currentStatus } : u));
            } else {
                toast.success(`User ${!currentStatus ? 'blocked' : 'enabled'}`);
            }
        });
    };
    ...
    const disabledUsers = users.filter(u => u.isBlocked).length;
                ...
                <div className="p-6 pt-2 overflow-y-auto flex-1">
                    ...
                    <div className="grid grid-cols-3 gap-4 mb-8">
                        <div className="flex flex-col items-center justify-center p-4 bg-white rounded-xl shadow-sm border border-slate-100">
                            <h4 className="text-sm font-medium text-slate-500 mb-1">Total User</h4>
                        ...
                        <div className="flex flex-col items-center justify-center p-4 bg-white rounded-xl shadow-sm border border-slate-100">
                            <h4 className="text-sm font-medium text-slate-500 mb-1">Active User</h4>
                        ...
                        <div className="flex flex-col items-center justify-center p-4 bg-white rounded-xl shadow-sm border border-slate-100">
                            <h4 className="text-sm font-medium text-slate-500 mb-1">Disable User</h4>
                    ...
                    <div className="space-y-3">
                        {loading ? (
                            ...
                            users.map((user) => (
                                <div key={user.id} className="flex items-center justify-between p-3 rounded-xl border border-slate-100 bg-white hover:border-slate-200 transition-colors">

...
const getPublicSiteConfig = unstable_cache(
  async (domain: string) => {
    ...
    try {
      ...
      if (!tenant && isTenantMode) {
        console.warn(
          `[getPublicSiteConfig] No tenant for ${domain}; falling back to first tenant in tenant mode.`,
        );
      ...
      return {
        ...
        customDomain: config.tenant?.customDomain || tenant.customDomain || "",
        slug: config.tenant?.slug || tenant.slug || "",
...
export async function getAdminSiteConfig() {
  ...
  try {
    ...
    if (!config) {
      console.log(
        `[getAdminSiteConfig] No config found for tenant ${tenantId}`,
      );
      return defaults;
    }
...
export async function updateSiteConfig<T extends object>(data: T) {
  ...
  if (!tenantId) {
    return {
      success: false,
      error: "Unauthorized: Tenant ID not found in session.",
    };
  }
  ...
  try {
    ...
    if (existing) {
      ...
      const updateResult = await updateTenantInfo(
        existing.tenantId!,
        existing.tenant,
        slug,
        customDomain,
...
export async function updateDeliveryConfig<T extends object>(data: T) {
  ...
  try {
    ...
    const payload = {
      ...
      defaultCodEnabled:
        input.defaultCodEnabled === undefined
          ? undefined
          : Boolean(input.defaultCodEnabled),
      nonRefundable:
        input.nonRefundable === undefined
          ? undefined
          : Boolean(input.nonRefundable),
      ...
      courierPathaoEnabled:
        input.courierPathaoEnabled === undefined
          ? undefined
          : Boolean(input.courierPathaoEnabled),
      courierPathaoCredentials:
        input.courierPathaoCredentials as Prisma.InputJsonValue,
...
function getSeoPayload(
  ...
  for (const field of seoFields) {
    ...
    if (fieldValue !== undefined) {
      if (
        isStandardOrHigher ||
        ["seoTitle", "seoDescription", "seoKeywords", "ogImage"].includes(field)
      ) {

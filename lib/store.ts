...
interface CartState {
  ...
  // Menu cache for fast navigation
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  menuProducts: any[];
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  menuCategories: any[];
  menuDomain: string | null;
  menuCacheAt: number | null;
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
    ...
    (set, get) => ({
      ...
      setMenuCache: (payload) =>
        set({
          ...
          menuDomain: payload.domain,
          menuCacheAt: payload.timestamp,
        }),
    }),
    {
      name: "crabkhai-cart",
      partialize: (state) => ({
        ...
        menuDomain: state.menuDomain,
        menuCacheAt: state.menuCacheAt,
      }),
    },
  ),
);
